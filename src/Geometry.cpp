...
Mesh<Vertex_Pos_Tex>* Geometry::CreateTessQuad(GLuint tess)
{	
    // Create the vertex buffer for a quad
    std::vector<Vertex_Pos_Tex> vb;
    vb.reserve(static_cast<size_t>(tess) * tess);

    for (GLuint j = 0; j <= tess; j++)
    {
        for (GLuint i = 0; i <= tess; i++)
        {
	        const float it = static_cast<float>(i) / tess;
            const float jt = static_cast<float>(j) / tess;
            vb.push_back({ it - 0.5f, 0.f, jt - 0.5f, it, jt });
        }
    }
    ...
    for (GLuint j = 0; j < tess; ++j)
    {
	    const GLuint offset{ j * (tess + 1) };
	    for (GLuint i = 0; i < tess; i++)
	    {
            const GLuint start{ offset + i };
		    ib.push_back(start);
		    ib.push_back(start + 1);
		    ib.push_back(start + tess + 1);
   
		    ib.push_back(start + 1);
		    ib.push_back(start + tess + 2);
		    ib.push_back(start + tess + 1);
	    }
    }    
